...
// when it is not, the Supervisor answers the same calls with its own state. The
// switch is the front door's and no client participates in it — which is the
// whole point, because the alternative is every client, on every platform,
// hand-coding a second source and a rule for choosing between them.
//
// It replaced exactly that. The Shell had a `useSupervisor` hook, a second
// fetch path, a poll, an EventSource and a branch deciding which of two trees to
// draw — roughly 150 lines that a Flutter client, a TV client and every future
// one would each have had to write again, correctly, from a description rather
// than from a contract.
//
...
// **Two types, because both services declare an Invoke** and one Go type cannot
// carry two methods of that name. They share the same state function and the
// same rule; splitting them costs a struct and keeps the generated interfaces
// as the thing being satisfied, which is what makes a contract change here a
// compile error rather than a route that quietly stops being answered.
...
//
// The doorway *is* the Supervisor's screen here. The Shell renders whatever
// this returns exactly as it renders the Platform's own door, so a client that
// opens the page mid-install sees the install rather than a sign-in form for a
...
//
// **A credential must never be answered by anything that cannot check it.** An
// Ack here would tell a client it had signed in when nothing verified anything,
// and a client is entitled to believe an answer on this route. Unavailable is
// the truth: there is a thing that can check it, and it is not running.
...
// SignOut is refused rather than acknowledged, which is the opposite of how the
// session intents are treated above and for a reason worth stating.
//
...
// screen as the shell, re-pushes it whenever it changes, and **ends the stream
// the moment the Platform is serving.**
//
...
//
// **seq stays zero, and that is load-bearing.** A client stores the last seq it
// saw and sends it back as the resume cursor on reconnect — to the *Platform*,
// which is the next thing it will reach. A cursor minted here would be a number
// from one server presented as a position in another's stream, and the Platform
// would honour it by replaying from a point that means nothing. Zero is "I have
// seen nothing", which is exactly true of the Platform's stream.
...
//
// **The one thing a client needs that a tree cannot carry.** Everything visible
// is in the screen and needs no interpretation, but a client may reasonably want
// to know it is not talking to Mosaic — to keep an "offline" affordance out of
// the way, or to leave a sign-out control alone. Naming it here means a client
// that wants that reads a field, and one that does not ignores an event type it
// does not know, which every client already does.
